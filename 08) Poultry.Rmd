...
- Table eggs: Eggs grown for sale to consumers (as opposed to fertilized eggs).
- Shell eggs: Eggs sold in cartons.
...
# All egg farms with production contracts (2012 census of agriculture)
...
```
\tiny
This includes farms that produce table eggs and fertilized eggs. This is a density dot map where each dot represents about 1 operation, randomly located within a county. Data source: @NASS.

# All egg farms with more than 10,000 layers (2012 census of agriculture)
...
# Table eggs production by state (2017)
...
kable(dta, booktabs = T) %>%
  kable_styling(latex_options = "striped") %>%
  footnote(general = "Million dozens")
...
dta_r <- dta_r %>%
  ...
  mutate(var = "Retail, white A, large shell egg ($/doz)")
...
- Almost all chicken production is under some form of vertical integration.
- Almost all egg production is under full vertical integration.
    - In the table egg production, farms control the whole supply chain.
    - No need for contracts, except perhaps for feeds, as production is fully integrated.
...
# Contracts for chicken production {.allowframebreaks}
...
# Disadvantages to vertical integration in the poultry industry
