...
- Like cattle, the characteristics of hogs are difficult to observe on a live animal and the market has come up with solutions for the adverse selection problem it can create.
- I will begin with definitions, review some market data and finally turn to the economics of hog marketing.
...
- Feeder-to-finish: Production of hogs from feeder pigs to finishing (market hogs).
...
plot_slaughter <- ggplot(dta, aes(x = year, y = Value)) +
  ...
  ylab("Hog slaughter (million head)") +
...
if(Sys.time() - file.info("Data/7) Pork.xlsx")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
# Average live and dressed hog weight in federally inspected facility

```{r weight, out.width = '75%'}

if(Sys.time() - file.info("Data/7) Weight.xlsx")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
```
\scriptsize
Note: I suspect that the drop in carcass weight in the 1970s is from a modifcation in the way to measure carcass weight. Data source: @USDA_dom.
...
# Operations with hog inventory by county (2012 census of agriculture)

```{r, out.width = '75%'}

# Read data
dta <- read_csv("Data/7) Hog inventory by county.csv")
dta <- dta %>%
...
```
\scriptsize
This is a density dot map where each dot represents about 5 operations, with larger dots representing groups of larger operations. Data source: @NASS.
...
# Operations with market hog inventory by county (2012 census of agriculture)
...
```
\scriptsize
This is a density dot map where each dot represents about 10 operations, irrespective of their sizes. Data source: @NASS.
...
```
\scriptsize
This is a density dot map. A dot does not represent the specific location of hog. Rather, the number of dots within a state that represents the relative hog slaughter by state. Data source: @NASS.
...
if(Sys.time() - file.info("Data/7) Hog trade.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
```{r, out.width = '75%'}
if(Sys.time() - file.info("Data/7) Pork trade.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
if(Sys.time() - file.info("Data/7) Livestock prices.xlsx")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
if(Sys.time() - file.info("Data/7) Meat prices.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
```
\scriptsize
Note: CWE means Carcass Weight Equivalent. Data source: @USDA_dom.
...
- Cycles occur because of biological nature of hog production:
...
- Biological lags cause a delayed response between a shock and a responde in production.
...
```
\scriptsize
Note: I suspect that the shocks in the 1930s and 1940s have to do with how the data were collected. Data source: @NASS.
...
if(Sys.time() - file.info("Figures/7)_Return_farrow_to_finish.png")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
  fig <- image_read("https://www.extension.iastate.edu/agdm/livestock/html/images/b1-31tbl1.gif")
  image_write(fig, path = "Figures/7)_Return_farrow_to_finish.png", format = "png")
}

knitr::include_graphics("Figures/7)_Return_farrow_to_finish.png")
```
\scriptsize
Source: @Schulz2018.
...
- See definitions in [this ERS publication](https://www.ers.usda.gov/webdocs/publications/40764/18614_aer747a_1_.pdf?v=41063).
- *Marketing contracts* refer to verbal or written agreements between a contractor and a grower that sets a price (or pricing mechanism) and an outlet for the commodity before harvest or before the commodity is ready to be marketed. 
- *Production contracts* specify in detail the production inputs supplied by the contractor (processor, feed mill, other farm operation or business), the quality and quantity of a particular commodity, and the type of compensation to the grower (contractee) for services rendered.
- Marketing contracts are common in grains and cattle industries.
- Marketing contracts are also common in the hog industry.
- There are also production contracts in the hog industry.
...
- There are contracts in the grain and cattle industry.
    ...
    - The contracts are typically for a relatively short period of time (i.e. a few months).
- Contracts in hog and poultry, as well as for specialty crops, are different:
    ...
    - The contracts cover longer time periods (several years).
...
- As transaction costs increase and assets are more specific, then contracts minimize costs.
    ...
    - This is the case for a major part of the hog production.
- In the case where transaction costs are high and assets are very specific, then full integration becomes the cost minimizing method.
    - Production contracts are an example of full integration.
    - This is the example of some of the hog production and almost all of the poultry and egg industry where packers control the supply chain from feeds up to packing.
...
- Most of the hog production is under some form of vertical integration (production contracts mean full integration).
...
- There are some disadvantages to vertical integration in the hog industry.
    ...
    - Growers retain production risk.
...
- Quality is difficult to observe but hogs tend to be more homogenous than cattle.
    - There is more breed variety in cattle (southern versus northern breeds, dairy).
...
- The number of hogs sold on a live weight basis is small.
...
if(Sys.time() - file.info("Data/7) Prior day hog slaughter.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
