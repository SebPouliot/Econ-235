...
d_inv <- ggplot() +
  ...
  geom_point(data = p_dta, mapping = aes(longitude, latitude, alpha = size), shape=20, color = "blue") + 
...
```
\scriptsize
This is a density dot map where each dot represents about 5 operations, with darker dots representing larger operations. Data source: @NASS.
...
d_inv <- ggplot() +
  geom_polygon(data=us_county_map, aes(x=long, y=lat, group = group), fill = NA, color = "gray") +
  geom_point(data = p_dta, mapping = aes(longitude, latitude), shape=20, color = "blue", alpha = 0.5, size = 0.25) + 
  scale_size_continuous(range = c(0.1, 1)) +
  geom_polygon(data=us_state_map, aes(x=long, y=lat, group = group), fill = NA, color = "black") +
  coord_map( "polyconic" ) +
  theme_bw() +
...
```
\scriptsize
This is a density dot map. A dot does not represent the specific location of cattle. Rather, the number of dots within a state that represents the relative cattle slaughter by state. Data source: @NASS.
...
if(Sys.time() - file.info("Figures/7)_Return_farrow_to_finish.png")$mtime > months(1, abbreviate = FALSE)){
...
- See definitions in <https://www.ers.usda.gov/webdocs/publications/40764/18614_aer747a_1_.pdf?v=41063>.
...
- There are also production contracts in the hig industry.
