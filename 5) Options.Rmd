...
<!-- dta = data.frame(array(NA, c(6,4))) -->
<!-- colnames(dta) <- c("Positions", "Trader's right", "Trader's obligation", "Margin required?") -->

<!-- dta[,1] <- c("Futures contract buyer (long)", "Futures contract seller (short)", "Put option buyer", "Put option seller", "Call option buyer", "Call option seller") -->

<!-- dta[,2] <- c("", "", "Sell futures at contract price", "", "Buy futures at contract price", "") -->

<!-- dta[,3] <- c("Accept commodity or financial asset at contract price or cash settle", "Deliver commodity or financial asset at contract price or cash settle", "", "Buy futures contract at strike price", "", "Sell futures contract at strike price") -->

<!-- dta[,4] <- c("Yes", "Yes", "No", "Yes", "No", "Yes") -->

<!-- kable(dta, format = "latex", caption = "Difference between futures contracts and options on futures", booktabs = T) %>%  -->
<!--   kable_styling(font_size = 7, full_width = F) #latex_options = "striped" -->
...
library(pacman)
suppressMessages(source("5) Option graphs.R"))
```
...
-  You expect the time value to decline over time.
