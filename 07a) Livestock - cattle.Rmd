...
if(Sys.time() - file.info("Data/7) Beef.xlsx")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
if(Sys.time() - file.info("Data/7) Cattle trade.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
```{r, out.width = '75%'}
if(Sys.time() - file.info("Data/7) Beef and veal trade.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
if(Sys.time() - file.info("Figures/7)_Return_finishing_steer.png")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
1. Live weight:
    ...
    - Buyer (packer) assumes yield and quality risk.
        - Adverse selection may cause a discounted price compared to other selling methods.
...
# Adverse selection in live versus carcass pricing {.allowframebreaks}
...
- This is an example of adverse selection where buyers discount the price of a good for which they cannot observe the quality.
...
if(Sys.time() - file.info("Data/7) Live versus dressed.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
if(Sys.time() - file.info("Data/7) Cattle - formula.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
...
if(Sys.time() - file.info("Data/7) Cattle - negotiated.xls")$mtime > months(1, abbreviate = FALSE) & pingr::is_online()){
